    ...
    private void pRequestHelper(Request request) {
        try {
            ...
            if (e.code().intValue() > Code.APIERROR.intValue()) {
                ...
                LOG.warn("Got user-level KeeperException when processing " +
            "sessionid:{} type:create cxid:{} zxid:{} txntype:{} reqpath:{} " +
            "Error Path:{} Error:KeeperErrorCode = NodeExists for {}",
            sessionId, cxid, zxid, txnType, reqPath, errorPath, nodePath);
            }
